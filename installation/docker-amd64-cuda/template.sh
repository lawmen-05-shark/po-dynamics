...
PROJECT_ROOT_AT=/project/po-dynamics
ACCELERATION=cpu
WANDB_API_KEY=
...
pull_generic() {
  ...
  if [ "${PULL_IMAGE_NAME}" == "" ]; then
    ...
    echo "For example: ./template.sh pull ic-registry.anonymous-institution.xyz/${LAB_NAME}/anonymous/${PROJECT_NAME}"
...
push_usr_or_root() {
  ...
  if [ "${PUSH_IMAGE_NAME}" == "" ]; then
    ...
    echo "For example: ./template.sh push docker.io/anonymousanonymous/po-dynamics"
    echo "ANONYMOUS-INSTITUTION people can just do ./template.sh push IC or ./template.sh push RCP
      And it will be pushed to ic-registry.anonymous-institution.xyz/${IMAGE_NAME}
      or registry.rcp.anonymous-institution.xyz/${IMAGE_NAME}"
    exit 1
  elif [ "${PUSH_IMAGE_NAME}" == "IC" ]; then
    PUSH_IMAGE_NAME="ic-registry.anonymous-institution.xyz/${IMAGE_NAME}"
  elif [ "${PUSH_IMAGE_NAME}" == "RCP" ]; then
    PUSH_IMAGE_NAME="registry.rcp.anonymous-institution.xyz/${IMAGE_NAME}"
  fi
...
get_runai_scripts() {
  ...
  check
  cp -r "./ANONYMOUS-INSTITUTION-runai-setup/template-submit-examples/" "./ANONYMOUS-INSTITUTION-runai-setup/submit-scripts"
  for file in \
    "./ANONYMOUS-INSTITUTION-runai-setup/submit-scripts/first_steps.sh" \
    "./ANONYMOUS-INSTITUTION-runai-setup/submit-scripts/minimal.sh" \
    "./ANONYMOUS-INSTITUTION-runai-setup/submit-scripts/remote_development.sh" \
    "./ANONYMOUS-INSTITUTION-runai-setup/submit-scripts/unattended.sh" ; do
