...
- To run on the ANONYMOUS-INSTITUTION Run:ai clusters, follow the instructions
  to [obtain/build the environment](#obtainingbuilding-the-environment)
  (perform them on your local machine)
  then refer to the `./ANONYMOUS-INSTITUTION-runai-setup/README.md`.
...
```bash
# SSH git@github.com:ANONYMOUS-Labo/po-dynamics.git
# URL https://github.com/ANONYMOUS-Labo/po-dynamics.git
git clone <URL/SSH> po-dynamics
cd po-dynamics
```
...
1. Create an environment file for your personal configuration with
   ...
   This creates a `.env` file with pre-filled values.
    - The `USRID` and `GRPID` are used to give the container user read/write access to the volumes that will be mounted
      ...
      (**ANONYMOUS-INSTITUTION Note:** _These should match the permissions on your lab's shared storage when mounting from there
      and running on some shared infrastructure, like HaaS setup with LDAP login or Run:ai.
      They will typically be your ANONYMOUS credentials.
    ...
    - `LAB_NAME` will be the first element in name of the local images you get.
      ...
      (**ANONYMOUS-INSTITUTION Note:** _If pushing to the IC or RCP registries this should be the name of your lab's project
      in the registry.
...
2. Pull or build the generic images.
   ...
   they will also be tagged with the latest git commit hash `run-<sha>-root` and `dev-<sha>-root`.
    - Pull the generic images if they're available.
      ...
      # For ANONYMOUS-INSTITUTION
      ./template.sh pull_generic registry.rcp.anonymous-institution.xyz/anonymous/anonymous/po-dynamics
      # Outside ANONYMOUS-INSTITUTION
      ./template.sh pull_generic docker.io/lawmen05shark/po-dynamics
      ````
...
is available at `docker.io/anonymousanonymous/po-dynamics`
...
E.g., you can mount it at `/project/po-dynamics` and specify `PROJECT_ROOT_AT=/project/po-dynamics`.
...
You can refer to the `ANONYMOUS-INSTITUTION-runai-setup/README.md` for an idea of how this would work on a Kubernetes cluster
