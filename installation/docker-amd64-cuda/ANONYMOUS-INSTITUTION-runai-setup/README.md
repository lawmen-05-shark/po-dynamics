# Guide for using the template with the ANONYMOUS-INSTITUTION IC and RCP Run:ai clusters

## Overview

At this point, you should have runtime and development images that can be deployed on multiple platforms.
This guide will show you how to deploy your images on the ANONYMOUS-INSTITUTION IC and RCP Run:ai clusters and use them for:
...
2. You should have one or more PVC(s) (Persistent Volume Claim) connecting some persistent storage
   ...
   (E.g. `runai-anonymous-anonymous-scratch`, you can run `kubectl get pvc` to list them).
3. You should have access to a project on the [IC](https://ic-registry.anonymous-institution.xyz/) or [RCP](https://registry.rcp.anonymous-institution.xyz/)
   image registries
...
They will be in `.ANONYMOUS-INSTITUTION-runai-setup/submit-scripts`.
...
3. Clone your repository in your PVC.
   ...
   mkdir po-dynamics
   git clone <repo-url> po-dynamics/dev
   git clone <repo-url> po-dynamics/run
   ```
   ...
   ```bash
   cd po-dynamics/dev && git config core.filemode false
...
...
[TEMPLATE INFO] Running entrypoint.sh
[TEMPLATE INFO] PROJECT_ROOT_AT is set to /anonymous-rcp-scratch/home/anonymous/po-dynamics/dev.
[TEMPLATE INFO] The next commands (and all interactive shells) will be run from /anonymous-rcp-scratch/home/anonymous/po-dynamics/dev.
[TEMPLATE INFO] Installing the project with pip.
[TEMPLATE INFO] Expecting /anonymous-rcp-scratch/home/anonymous/po-dynamics/dev to be a Python project.
[TEMPLATE INFO] To skip this installation use the env variable SKIP_INSTALL_PROJECT=1.
...
Obtaining file:///anonymous-rcp-scratch/home/anonymous/po-dynamics/dev
...
Building wheels for collected packages: po-dynamics
...
Successfully built po-dynamics
Installing collected packages: po-dynamics
Successfully installed po-dynamics-0.0.1
...
...
```
/anonymous-rcp-scrach/home/anonymous/po-dynamics
...
1. Same as option 1, but set the environment variable `PYCHARM_IDE_AT` to the directory containing the IDE binaries.
   ...
   ```bash
    Gateway link: jetbrains-gateway://connect#idePath=%2Fanonymous-rcp-scratch%2Fhome%2Fanonymous%2Fremote-development%2Fpycharm&projectPath=%2Fanonymous-rcp-scratch%2Fhome%2Fanonymous%2Fpo-dynamics%2Fdev&host=127.0.0.1&port=2222&user=anonymous&type=ssh&deploy=false&newUi=true
    ```
2. Enable port forwarding for the SSH port.
