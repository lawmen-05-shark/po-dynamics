...
def compute_eval_stats(batches):
    """Computes the max timestep and max/avg return of the batch."""
    out = {
    ...
    if batches["next", "done"].any():
        ...
        avg_return = (batches["next", "return"] * batches["next", "done"]).sum() / batches["next", "done"].sum()
        out.update({f"perf/max_return": max_return, f"perf/avg_return": avg_return})
        ...
        if ("next", "return_raw") in batches.keys(include_nested=True):
            ...
            avg_return_raw = (batches["next", "return_raw"] * batches["next", "done"]).sum() / batches[
                "next", "done"
            ].sum()
